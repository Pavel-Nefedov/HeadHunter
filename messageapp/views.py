...
from mainapp.mixins import IsModeratorCheckMixin, IsCandidateCheckMixin, IsCompanyCheckMixin
...
class ResponseToVacancyView(IsCandidateCheckMixin, View):
    def get(self, request, user_id, vacancy_id):
        ...
        with transaction.atomic():
            ...
            current_message = f"На вашу вакансию <a href='{reverse('company:vacancy', kwargs={'pk': vacancy_id})}'>" \
                              f"{vacancy.vacancy_name}<a> поступил отклик от пользователя, его резюме {request.user.first_name} " \
                              f"{request.user.last_name}  <a href='{reverse('candidateapp:resume_detail', kwargs={'pk': resume_form_id})}'>{resume.desired_position}<a>."
...
@method_decorator(login_required, name='dispatch')
class ResponseToResumeView(IsCompanyCheckMixin, View):
    def get(self, request, user_id, resume_id):
        ...
        with transaction.atomic():
            ...
            if 'radio' in request.GET:
                vacancy_form_id = request.GET['radio']
            else:
                return redirect(reverse('company:vacancy', kwargs={'pk': resume_id}))

            resume = Resume.objects.get(pk=resume_id)
            profile = CompanyProfile.objects.get(user=request.user)
            vacancy = Vacancy.objects.get(company=request.user)


            current_message = f"На ваше резюме <a href='{reverse('candidate:resume_detail', kwargs={'pk': resume_id})}'>" \
                              f" {resume.desired_position}<a> поступил отклик от компании {profile.company_name} " \
                              f" <a href='{reverse('company:vacancy', kwargs={'pk': vacancy_form_id})}'>её вакансия ({vacancy.vacancy_name})<a>."
            redirect_url = reverse('company:resume_search')
